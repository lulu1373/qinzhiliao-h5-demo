(function (root, factory) {
  ...
  function result(type, draft, e) {
    const c = copy[type];
    if (!draft.flipped) return `<div class="cp-intro"><h2>给刚刚写下的，留一点空间</h2><p>准备好了，再翻开看看。</p></div><div class="cp-motion-host"><i class="cp-practice-shadow" aria-hidden="true"></i><button type="button" class="cp-card cp-card-back cp-motion-back" data-xp-action="card-flip" data-type="${type}" aria-label="翻开${c.name}"><span class="cp-card-frame">${mark(c)}<strong>${c.name}</strong></span></button></div>${button('card-edit','先改一改',type,'cp-link cp-centered')}`;
    const card = draft.card;
    return `<div class="cp-motion-host"><i class="cp-practice-shadow" aria-hidden="true"></i><article class="cp-card cp-card-front cp-motion-front"><div class="cp-card-meta">${mark(c)}<span>${c.name} · 由我写下</span></div><h2>${e(card.title || c.card)}</h2>${(Array.isArray(card.summary) ? card.summary : []).map(line=>`<p>${e(line)}</p>`).join('')}</article></div><div class="cp-result-actions">${draft.saved ? '<p class="cp-saved" role="status">已存入百宝箱</p>' : button('card-save','确认存入百宝箱',type,'cp-primary')}${draft.saved ? button('card-new','再记一张',type,'cp-primary') : ''}${button('card-edit','修改这张卡',type)}<button type="button" class="cp-link" data-xp-action="route" data-route="treasure-box">去百宝箱看看</button></div>`;
  }
