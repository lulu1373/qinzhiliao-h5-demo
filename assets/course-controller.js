...
function installCourse() {
  ...
  const context = () => ({data:read(),model,esc});
  function read() {
    ...
    const membership = {...inherited,active:!!(inherited.active || state.membership?.active),cycleId:inherited.cycleId || day(),quota:2};
  ...
  function settle(id, result) {
    ...
    toast(result === 'success' ? '课程已加入我的学习' : result === 'processing' ? '正在确认结果' : '这次模拟支付未完成');
  }
  ...
  function demoState(kind) {
    ...
    if (kind === 'member') { commit(model.normalize({membership:{active:true,cycleId:today,quota:2,used:0}}),{membership:{...state.membership,active:true}}); }
  ...
  function showRecommendation() {
    ...
    const product = model.CATALOG.find(item => item.id === 'M203');
    return `<aside class="course-chat-recommendation"><small>相关课程 · 可忽略</small><b>${esc(product.title)}</b><p>当你想把“规则”和“连接”一起放进日常，这节示例课可以作为一个起点。</p><div><button data-course-action="detail" data-product-id="${product.id}">看看课程</button><button data-course-action="hide-recommendation">暂时不看</button></div></aside>`;
  }
  ...
  const oldHomeIdle = renderHomeIdle;
  renderHomeIdle = function() { return `${oldHomeIdle()}${view.homeEntry(context())}`; };
  const oldChat = renderChat;
  renderChat = function() { return `${oldChat()}${showRecommendation()}`; };
  const oldConfirmSummary = handleConfirmOk;
  handleConfirmOk = function() {
    oldConfirmSummary();
    const grant = earn('chat_summary',state.chat?.conversationId || day(),4);
  ...
  renderDrawer = function() {
    oldDrawer();
    const grid = drawerEl.querySelector('.v6-drawer-feature-grid');
    if (grid && !grid.querySelector('[data-course-action="route"]')) grid.insertAdjacentHTML('beforeend',view.drawerEntry(context()));
  };
  document.addEventListener('click',event => {
    ...
    try {
      if (action === 'route') go(el.dataset.route || 'courses');
      else if (action === 'back') go(el.dataset.fallback || 'courses',true);
