...
class GrowthReportTests(unittest.TestCase):
    ...
    def test_latest_preloads_complete_fixed_conversation(self):
        ...
        self.page.wait_for_selector('[data-action="fx-accept"]')
        self.assertIn(seed,self.page.locator('.v36-conversation').inner_text())
        self.assertEqual(self.page.locator('.fx-stage').count(),0,'progress stays hidden before the parent opts in')
        self.page.locator('[data-action="fx-accept"]').click()
        self.assertIn('好，帮我看看吧。',self.page.locator('.v36-conversation').inner_text())
        self.assertIn('一起看懂这次发生的事',self.page.locator('.fx-stage').inner_text())
        for _ in range(2):
            self.page.locator('[data-action="fx-example"]').click()
            self.page.locator('#chatSendBtn').click()
        self.assertIn('如果有不准确或遗漏的地方，你可以进行补充。',self.page.locator('.v36-conversation').inner_text())
        self.assertEqual(self.page.locator('.fx-actions button').all_inner_texts(),['对，就是这样','补充或修改'])
        self.page.locator('[data-action="fx-confirm"]').click()
        self.page.locator('[data-action="fx-example"]').click()
        self.page.locator('#chatSendBtn').click()
        self.assertIn('这次解读已整理好',self.page.locator('.fx-stage').inner_text())
        self.assertIn('① 这次发生了什么',self.page.locator('.fx-report').inner_text())
        self.page.locator('[data-action="fx-claim"]').click()
