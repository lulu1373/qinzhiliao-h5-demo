...
class ExperienceTests(unittest.TestCase):
    ...
    def test_home_inline_composer_can_start_free_chat_and_moves_to_bottom(self):
        self.page.goto(self.url + '#/home')
        self.page.wait_for_selector('.xp-home-chat-start #chatInput:visible')
        self.page.locator('#chatInput').fill('今天回家以后，我不知道该先说什么。')
        self.page.locator('#chatSendBtn').click()
        self.page.wait_for_selector('.v36-conversation.active')
        self.assertIn('今天回家以后', self.page.locator('.v36-conversation').inner_text())
        self.assertEqual(self.page.locator('#chatInput').count(), 1)
        self.assertEqual(self.page.locator('.xp-home-chat-start').count(), 0)
        self.assertEqual(self.page.locator('.ai-shell > .composer').count(), 1)
        self.assertEqual(self.page.locator('.ai-shell > .composer .v90-card-pill').count(), 5)

    def test_home_conversation_first_hierarchy_and_single_composer(self):
        self.page.goto(self.url + '#/home')
        self.page.wait_for_selector('.xp-home-chat-start #chatInput:visible')
        self.assertEqual(self.page.locator('#chatInput').count(), 1)
        self.assertEqual(self.page.locator('.ai-shell > .composer').count(), 0)
        self.assertIn('今天想先说点什么？', self.page.locator('.xp-home-hero').inner_text())
        self.assertNotIn('小亲在这里', self.page.locator('.xp-home').inner_text())
        hero = self.page.locator('.xp-home-hero').bounding_box()
        composer = self.page.locator('.xp-home-chat-start .composer-row').bounding_box()
        starters = self.page.locator('.xp-home-starter-section').bounding_box()
        self.assertLessEqual(hero['height'], 130)
        self.assertGreater(composer['y'], hero['y'])
        self.assertLess(composer['y'], starters['y'])
        self.assertLess(composer['y'] + composer['height'], 620)
        self.assertEqual(self.page.locator('.xp-home-starters > button').count(), 3)
        self.assertIn('刚刚发生了一件事', self.page.locator('.xp-home-starters').inner_text())
        self.assertEqual(self.page.locator('.xp-home-primary > button').count(), 2)
        self.assertEqual(self.page.locator('.xp-home-card-tools .v90-card-pill').count(), 5)
