...
ROOT=Path(__file__).resolve().parents[1];HTML=(ROOT/'dist/index.html').read_text(encoding='utf-8')
...
async def run():
 ...
 (ROOT/'tests/back_button_audit_v34.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8');print(json.dumps(payload['summary'],ensure_ascii=False))
