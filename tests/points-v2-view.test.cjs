...
test('renders all points v2 routes',()=>{
  const cases=[
    ['points','成长中心'],['points/checkin','每日签到'],['points/calendar','签到日历'],['points/tasks','任务中心'],
    ['points/ledger','积分明细'],['points/rewards','积分兑换'],['points/level','我的等级'],['points/rules','积分规则']
  ];
...
test('dashboard exposes balance, date checkin, tasks, ledger and rewards',()=>{
  ...
  assert.match(html,/126/);assert.match(html,/9月18日/);assert.match(html,/points\/tasks/);assert.match(html,/points\/ledger/);assert.match(html,/points\/rewards/);
});
