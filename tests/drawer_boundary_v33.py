...
OUT=ROOT/'tests/drawer-v33'
OUT.mkdir(exist_ok=True)
RESULT={'version':'V3.3','environment':'Chromium headless; browser mobile-size rendering, not physical iOS/Android devices','cases':[],'page_errors':[],'console_errors':[]}
...
baseline=Path('/mnt/data/亲智聊-H5-App-Demo-V3.2-无边缘残影版.html')
RESULT['source_v32_unchanged']=(sha256(baseline.read_bytes()).hexdigest()=='e5233dd5d361bfe60a2f18a145831c3960f0f65f267a7e1c371d1bcd9414b0b5') if baseline.exists() else None
(OUT/'result.json').write_text(json.dumps(RESULT,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'passed':RESULT['passed'],'pixel_identical':RESULT['pixel_isolation']['identical'],'page_errors':RESULT['page_errors'],'console_errors':RESULT['console_errors'],'v32_unchanged':RESULT['source_v32_unchanged']},ensure_ascii=False))
