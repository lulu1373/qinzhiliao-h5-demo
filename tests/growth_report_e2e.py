...
class GrowthReportTests(unittest.TestCase):
    ...
    def test_latest_preloads_complete_fixed_conversation(self):
        ...
        self.assertEqual(self.page.locator('.fixed-stage').count(),0,'progress stays hidden before the parent opts in')
        self.page.locator('[data-action="fixed-accept"]').click()
        self.assertIn('好，帮我看看吧。',self.page.locator('.v36-conversation').inner_text())
        self.assertIn('一起看懂这次发生的事',self.page.locator('.fixed-stage').inner_text())
        self.page.locator('#chatSendBtn').click()
        self.page.locator('#chatSendBtn').click()
        self.assertIn('如果有不准确或遗漏的地方，你可以进行补充。',self.page.locator('.v36-conversation').inner_text())
        self.assertEqual(self.page.locator('.fixed-quick-replies button').all_inner_texts(),['对，就是这样','补充修改'])
        self.assertEqual(self.page.locator('#chatInput').input_value(),'','composer stays empty while confirming the summary')
        self.page.locator('[data-action="fixed-confirm"]').click()
        self.page.locator('#chatSendBtn').click()
        self.assertIn('这次解读已整理好',self.page.locator('.fixed-stage').inner_text())
