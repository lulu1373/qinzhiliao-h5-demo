"""V3.5 browser regression. No external service calls.
...
OUT=R/'tests/screens-v35-regression'; OUT.mkdir(exist_ok=True)
RESULT={'version':'V3.5','environment':'Chromium headless, 390x844; set_content standalone; not physical devices','checks':[],'page_errors':[],'console_errors':[]}

def mark(name):
    RESULT['checks'].append({'name':name,'status':'PASS'})
    (R/'tests/navigation-centers-v35.json').write_text(json.dumps(RESULT,ensure_ascii=False,indent=2))
...
def drawer(pg,r):
    if pg.locator('#drawer').get_attribute('data-state')=='closed':pg.locator('[data-action="drawer-open"]').click()
    pg.locator(f'#drawer [data-route="{r}"]').click();rt(pg,r)

def back(pg,r):
    pg.locator('#screenHost .page-titlebar .back-btn').first.click(position={'x':3,'y':24});rt(pg,r)
    # V3.5 intentionally restores the origin drawer; close it for the next independent case.
    if r=='home' and pg.locator('#drawer').get_attribute('data-state')!='closed':
        pg.locator('#drawer [data-action="drawer-close"]').click()
